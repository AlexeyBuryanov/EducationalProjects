...
-- ПРИМЕЧАНИЕ #1: Защита на повторное создание написано только для БД потому, что, если БД есть, 
--                то она просто пересоздаётся, и уже отпадает потребность проверять что-либо ещё т.к. 
--                после пересоздания в БД тупо пусто, а потому лишние проверки на создание 
--                или пустоту таблиц избыточны (как по мне).
-- ПРИМЕЧАНИЕ #2: Скорректируйте правильный путь к фото, иначе сценарий не отработает.
